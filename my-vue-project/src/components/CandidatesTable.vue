                      ...
                      <tr>
                          ...
                          <td>
                              <div class="actions-wrapper">
                                  <button v-if="candidate.publication" type="button" class="candidate-icon candidate-show" @click="publicationEdit({ index })">
                                  ...
                                  <button v-if="candidate.researchgate_url =='' &&  candidate.googlescholar_url =='' " class="candidate-icon candidate-edit" @click="candidateEditUrl({ id: 'candidate-id-'+ index})">
                                      <svg fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" aria-hidden="true">
                                      <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <ul class="list list--notfound">
                      <li v-for="(pub,index) in notFoundPublications" :key="index" >
                        ...
                        <div class="cbox-wrapper">
                          <input type="checkbox" v-model="pub.checked" @change="updateNotFoundPublications(pub,this.localCandidates[this.localIndex].candidate._id)">
    ...
    methods: {
      ...
      saveUrl({ id }) {
        ...
        axios.post('http://127.0.0.1:8000/api-endpoint/Url', formData, {
          ...
          .catch(error => {
            this.loading = false;
            console.error("There was an error find the User", error);
            this.errorMessage = error.response ? error.response.data.message : 'There was an error find the User';
            this.showErrorPopup = true;
            this.hideSpinner()
          });
      ...
      submitCandidates() {
        this.showSpinner("We are calcculating the Ranking of the Candidates.It will take some time.");
